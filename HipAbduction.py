...
class HipAbduction(Exercise):
    def __init__(self):
        super().__init__()
        self.state_machine = ExerciseStateMachine("Hip Abduction", "hip", 30, 10)  # Eşik değerleri ayarlanır
    ...
    def validate_angles(self, keypoints):
        ...
        # Açı hesaplama
        hip_abduction_angle = self.calculate_angle(hip, knee, shoulder)

        # Eşik değerler ile kontrol
        if hip_abduction_angle < 30:
            return False  # Yanlış
        elif 30 <= hip_abduction_angle <= 45:
            return True  # Doğru
        else:
            return False  # Yanlış
